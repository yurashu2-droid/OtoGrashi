...
Arrangement arrangePerformance({
  ...
  final root =
      (((lead.registerMidiNote ?? lead.fundamentalMidiNote)?.round() ?? 57) - 3)
          .clamp(45, 76);
  ...
  SoundEvent copy(SoundEvent e, Map<String, Object?> changes) {
    ...
    if (values['pitchSteps'] case final List<Object?> curve) {
      values['pitchSteps'] = [
        for (final value in curve)
          if ((value as Map)['offsetSamples'] as int < duration)
            {
              ...value.cast<String, Object?>(),
              'durationSamples': math.min(
                value['durationSamples'] as int,
                duration - (value['offsetSamples'] as int),
              ),
            },
      ];
    }
  ...
  void hit(
    ...
    bool tail = false,
    AudibleRegion? passage,
  }) {
    if (start < 0 || start >= total || duration <= 0) return;
    final span = passage ?? part(clip, partIndex);
    ...
    for (var bar = 0; bar < bars; bar++) {
      ...
      if (bar >= leadEntry) {
        ...
        hit(
          ...
          gain: .47,
          steps: [
            PitchStep(offsetSamples: 0, durationSamples: 45000, midiNote: n),
            PitchStep(
              offsetSamples: 45000,
              durationSamples: 45000,
        ...
        if (mode == PerformanceMode.neonTune && e.pitchSteps.isNotEmpty) {
          ...
          for (var t = 0; t < e.durationSamples; t += 11250) {
            curve.add(
              PitchStep(
                offsetSamples: t,
                durationSamples: math.min(11250, e.durationSamples - t),
  ...
  if (animated && mode != PerformanceMode.loopStation) {
    for (var section = 0; section < seconds ~/ 15; section++) {
      for (var cue = 0; cue < 2; cue++) {
        ...
        final revealed = AudibleRegion(
          startSample: span.startSample,
          durationSamples: phraseLength,
        );
        ...
        for (var r = 0; r < 2; r++) {
          hit(
            ...
            gain: r == 0 ? .31 : .20,
            note: melodyTemplate == MelodyTemplate.answer
                ? (root + (r == 0 ? 2 : 7)).toDouble()
                : null,
            tail: true,
            passage: revealed,
          );
  ...
  if (mode == PerformanceMode.vinyl) {
    for (var group = 0; group < seconds ~/ 5; group++) {
      ...
      final sourceNote =
          (clip.registerMidiNote ?? clip.fundamentalMidiNote ?? 57).clamp(
            29.0,
            93.0,
          );
