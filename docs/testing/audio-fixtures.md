...
and its first decoded audio packet begins at a nonzero presentation timestamp.
AVFoundation reports the track interval origin as sample 0 because the container
has a leading empty edit. The authored impulse marker is at presentation sample
19,115 on the 48 kHz clock: the 0.1482199546-second input offset plus the source
impulse at 0.25 seconds, rounded to the nearest sample. AAC priming, resampler
ringing, and empty-edit materialization can change exact decoded packet coverage
and the first analysis frame across decoder versions. Native tests therefore
require decoded coverage to contain the authored marker and require the detected
onset's 480-sample analysis frame to contain that marker. Uncovered timeline
regions must remain zero. The lower-level placement regression uses synthetic
fixed PTS values to catch implementations that concatenate decoded buffers while
discarding presentation timestamps.
