...
録画操作とインカメ切替を同じ行に置き、撮影後は再生・撮り直し・採用を画面下に固定した。390×844の描画で両方の操作が見えることを確認した。採用後の動画コピー中は収集画面に進捗表示を重ね、重複操作を防ぐ。保存が完了して素材が増えた時だけ短い触覚フィードバックを返す。次の実機確認では、撮影→プレビュー→撮り直し→採用の流れと、追加中の待ち時間を評価する。
