...
struct VideoRenderer {
  ...
  private func drawFrame(
    ...
    if request.arrangement.performanceMode != "natural" {
      try await drawPerformanceFrame(frame, request: request, providers: providers,
        waveformPeaks: waveformPeaks, into: buffer, width: width, height: height)
      return
    }
  ...
  private func drawPerformanceFrame(_ frame: Int, request: VideoRenderRequestPayload,
    providers: [String: SourceProvider], waveformPeaks: [CGFloat], into buffer: CVPixelBuffer,
    width: Int, height: Int) async throws {
    let sample = frame * 1600
    let mode = request.arrangement.performanceMode
    let w = CGFloat(width), h = CGFloat(height)
    let bounds = CGRect(x: 0, y: 0, width: w, height: h)
    let sorted = request.arrangement.videoEvents.sorted { $0.destinationStartSample < $1.destinationStartSample }
    ...
    for (index, identity) in keys.enumerated() {
      ...
      let curve = request.arrangement.events.first { $0.assetId == event.assetId &&
        $0.destinationStartSample == event.destinationStartSample && ($0.partIndex ?? 0) == (event.partIndex ?? 0) }?.pitchSteps ?? []
      ...
      cards.append(PerformanceCard(event: event, playing: isPlaying, rect: tile, index: index, age: age))
    }
  ...
  private func drawPerformanceOverlay(_ cards: [PerformanceCard], frame: Int,
    ...
    for card in cards {
      ...
      let name = request.video.clipNames[card.event.assetId] ?? "SOUND \(card.index + 1)"
      ...
      if card.playing && (mode == "sampler" || mode == "neonTune" || mode == "loopStation") {
        ...
        // Radial visualizer follows rendered audio peaks, only on live pads.
        for k in 0..<40 {
          let a = Double(k) * Double.pi * 2 / 40
          let level = peaks.isEmpty ? CGFloat(0) : peaks[max(0, min(peaks.count - 1, frame + k / 5 - 4))]
